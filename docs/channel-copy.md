...
`npm run dry-run`（検証用サンプル: 2026-09-14 分、5 本）の実際の出力。**TOP・トップ・ランキング・◯位 は使わない。人気・勢いを示す語（話題 / 人気 / 注目 / 定番 / 急成長 / 急上昇 / バズ / 殿堂 / 最も / みんなが / 評価が高い / 高評価。英語の trending / viral / popular / hot（大文字小文字・全角を問わず）は、**そのツール自身の名前をそのまま丸ごと書く場合**（名前が "Hot Reload" のツールの説明に「Hot Reload に対応」）だけが例外で、それ以外はすべてエラー。「trending now」「Popular AIツール」「Trending Fonts」のような言い回しは大文字で書いても通らず、他のツールの名前・名前の一部・名前に語を足したものもエラー）も単独で使わない**（どちらも原稿の検証でエラーになる。ツール名そのものだけは例外）。
...
**2026-09-22 追記**: 上の「アカウント名（2026-09-16 オーナー決定: 変えない）」は、同 09-22 のオーナー指示（名前・アイコン・プロフィール文の一式を案として作る）により [`docs/account-identity.md`](account-identity.md) に引き継がれた。表示名 / IG の @ / プロフィール文 / アイコンの最新の案はそちらを見る（採用と設定変更はオーナー）。
