...
`npm run dry-run`（検証用サンプル: 2026-09-14 分、5 本）の実際の出力。**TOP・トップ・ランキング・◯位 は使わない。人気・勢いを示す語（話題 / 人気 / 注目 / 定番 / 急成長 / 急上昇 / バズ / 殿堂 / 最も / みんなが / 評価が高い / 高評価。英語の trending / viral / popular / hot は、直後に大文字で始まる語が来ない形（「trending now」「viral」など）だけ）も単独で使わない**（どちらも原稿の検証でエラーになる。ツール名そのものだけは例外）。
