...
- `name` はスナップショットの `name`（Product Hunt の名前）のまま。短くする・言い換える・順位の言葉を削るのは NG（当日向けのスナップショットと照合され、名前が合わないと検証エラー）。**英語の人気語（popular / trending / hot / viral）を名前に足すのも NG**（Product Hunt の名前に無い人気語が `name` にあると検証エラー。"Hot Canva" は不可、Product Hunt の名前が "Hot Reload" ならそのまま可。スナップショットが無いときは人気語を含む名前はすべてエラー）。名前が使えないツールは除外する
