...
| `pickup` | `days[0].posts`（`order` 順。`inAiCategory: true` が AI カテゴリ。`isAI` はキーワードからの目安） | `mode: "pickup"` |

- 各 post の `phUrl` が `ph_url` に使う URL、`name` / `tagline` が候補の名前と説明（英語）
...
- `ph_url` にはスナップショットの `phUrl`（クエリ文字列なし）をそのまま使う。`website` に Product Hunt の URL を入れると検証で NG になる
...
# 着弾確認（必須）— スナップショットのコミットにも日付が入るので、今日の原稿コミットの件名で確かめる
git fetch origin main --quiet
git log origin/main -10 --format=%s | grep -F "AIツールTOP5 $TODAY" \
