...
- **2026-09-14 ジャンル実験層を追加**（正本: sns-hub `docs/strategy/genre-experiment.md` / 写し: このリポ `docs/strategy.md` 冒頭の「ジャンル実験」節）。2026-05-12 版の trigger 指示文からの差分は次のとおりで、それ以外は 2026-05-12 版のまま:
  ...
  5. 手順 2 に配信死亡モードの例外（80/20 を使わない場合の選び方・凍結の解除）を追加し、手順 7 に「提案を JSON に反映しない」注意を追加（選定の制約の凍結ルールと「Explore 寄りで OK」にも配信死亡モードの例外を注記）
...
**a) 過去パフォーマンスを読む**
- `data/performance-history.json` から過去 14 日の `stats.views` (YT) / `instagram.views`・`instagram.saved` (IG。`instagram` か `instagram.views` が null の回は IG 側から除外) / `stats.likes` / `title` / `discovery.method` を抽出 (未運用期間中は空 OK) (これはネタ・method 分析用の直近 14 日で、0) の判定窓とは別。method 比較は型の初回投稿日 F 以降の回だけで行う)
...
- データがまだ薄い時期は Explore 寄り(新 method を試す)で OK (配信死亡モードの例外は手順 2 に従う)

**e) 今日の改善アクションを 3 つまで決める** (戦略のコンテンツ柱比率、勝ち筋 method 継続 or 新 method 試行を考慮。**配信死亡モードのアカウントについては method のアクションを書かず、構造実験 (タイトル個別化 / 型変更 / ジャンル変更) を「何を変えるか / 何で測るか / 14 日後の合格ライン」で提案する** (同じ動画を IG と YT に出しているので、動画そのものを変える提案は `IG / YT 共通` の 1 行にまとめてよい)。構造実験の提案と次ジャンル候補は**レポートに書くだけ**で、当日の原稿・タイトル・JSON には反映しない。Web 調査に時間を使わず短く書く。このリポの制約: ジャンル・型・タイトル方針のすべてを変えてよい)
...
- WORST method が 2 週連続なら一旦凍結 (配信死亡モードのアカウントの数字では凍結しない)
