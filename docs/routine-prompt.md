...
`pdca-summary.mjs` の出力に「今日の判定（今日は第 N 期 … の判定日）」の行が出ている日だけ、`docs/strategy.md` の「合格ライン」の閾値で「ジャンル判定」節を書く（判定は 14 日ごと。判定日の翌日からは次の期が始まり、判定の行は出ない。sns-hub のファイルはこのルーチンからは読めない）。
