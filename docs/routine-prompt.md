...
| `pickup` | **2〜5 本**（新作の範囲に入った候補から、使えるものがある分だけ） | おすすめ順（`ph_rank` は入れない） | 「新作AIツール N選」。**「TOP」「トップ」「ランキング」「◯位」は一切使わない** |

- **pickup で使える候補が 2 本に満たない日は投稿しない**: `data/enriched-ai-tools.json` を次の「休止」の形で書き、PDCA レポートと一緒に PR で入れる。パイプラインはこれを見て、エラーにせず動画を作らずに終わる。最終レポートにも「新作の候補が N 本のため休止」と書く
...
**文字のフィールド（name / description / who / pricing_note / narration / opening_narration）に入れてはいけないもの**: URL（`https://` や `www.`）、ドメイン名（`example.com` など。ツール名そのものに含まれる場合の `name` だけは可）、`@` で始まるメンション、`#` で始まるハッシュタグ、改行、制御文字、ゼロ幅文字。Product Hunt のタグラインや公式サイトの文章をコピーするときも、これらを取り除く。
