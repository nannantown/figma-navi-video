...
- **会社の主張は主張として書く**: 速度・料金の倍率、「ハルシネーションしない」「型エラーが起きない」などは、同じ文の中に「TypeSafe によると」「〜の検証では」のように**誰の話か**を入れる。事実として断定しない。第三者の検証（実測）があれば、それも出典つきで併記する
...
- **新情報の回（`news`）**: 前回の回の日付以降に公開され、まだ `role: "news"` で使っていない情報を 1 つ（`role: "news"`、`published_at` は記事の公開日）。同じ出来事を別の媒体が報じたものは同じ話題なので、前に出した話題（`topic_key`・`topic`）と重なるなら使わない
