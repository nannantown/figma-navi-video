...
- **オーナー決定 2026-09-23 A: CI でも 1 行手直しを当てる**: 上の件の直し方。`fetch-jev-x.yml` の取得ジョブで、twitter-cli を入れた直後のステップ「Patch twitter-cli (x.com -> x.com/home, one line)」が、venv の `twitter_cli/client.py` の `"https://x.com", headers=ct_headers,` を `"https://x.com/home", headers=ct_headers,` に置き換える（オーナーの Mac と同じ 1 行）。置き換える前に「対象がちょうど 1 か所・`x.com/home` は 0 か所」、置き換えた後に「対象 0 か所・`x.com/home` 1 か所」を確かめ、外れたら run を赤にして止める（版上げで黙って手直しが外れたり、別の行を書き換えたりしないため）。このステップは cookie を受け取らない。
  - **2026-09-23 追記: CI では手直し後も検索が 404**（PR #162 の検証 run 35818337819。手直しは当たっている）。ローカルでは twitter-cli のキャッシュ（`~/.twitter-cli/transaction_cache.json`、1 時間有効）を使わない状態でも、手直し後は初期化に成功して検索が通るので、キャッシュのせいではなく CI 環境側（GitHub のサーバーから見た X の応答）の違い。切り分けのため、①すぐ後の「Diagnose X search header init (no cookie)」ステップが cookie なしで `x.com/home` を取り、HTTP ステータス・バイト数・`ondemand.s` の有無・初期化の成否だけをログに出す（失敗しても run は赤にしない）②`errors` には twitter-cli の WARNING 行（`Failed to init ClientTransaction: …` など）も ` [warning: …]` として続けて入る。
