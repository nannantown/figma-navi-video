...
| 前の回に投稿の記録が無い（`performance-history.json` に YouTube・Instagram どちらの記録も無い） | **記録が無いだけでは「放送済み」のまま**（`next` の `unrecorded` に出る）。投稿は成功したのに記録だけ失敗した日にやり直すと二重投稿になるため。`node scripts/jev.mjs aired-check`（日付は渡さない。いつも `next` の `redoCandidate` = 最新の回の日を調べる）が、その日の投稿ワークフローの**全ての試行（再実行を含む）**のログに投稿成功の行（YouTube「Uploaded!」／Instagram「Published!」）が無く、どの試行も成功か失敗で終わっていることを確かめて `not-posted` と出たときだけ、**aired-check が出す `redoSlot`** の枠を `redo_of` 付きで書く（`redoSlot` を出すのは aired-check だけ）。途中で打ち切られた実行（cancelled / timed_out など）や、アップロードを始めてから失敗した実行（「YouTube: uploading」「Instagram: uploading Reel」の後に成功行が無い）がある日は、実際には届いている可能性があるので `unknown`。後日に `post-today-instagram.yml` を `date` 指定で回した分も、そのログの「Using date:」がその日なら数える。`jev.mjs validate` も `redo_of` の日をもう一度調べ、`not-posted` でなければ NG にする（記録の抜けが 2 日続いた日に、前の日の判定で最新の回をやり直す二重投稿を防ぐ）。08:15 の動画生成（`generate-data.mjs`）はこのログ確認をしない（`daily-video.yml` に Actions の読み取り権限が無いため。足すならワークフローの権限変更として別に判断する）。`redo_of` で指された回は**期限なく永久に**シリーズから外れる（時間で戻ることはない）。やり直せるのは最新の回だけで、`posted` / `unknown` のときや、もう次の回が出ているときはやり直さない（レポートに書く） |
...
| `redo_of`（任意） | 投稿されなかった最新の回をやり直すときだけ、その回の日付。`aired-check` が `not-posted` と出して `redoSlot` を出したときに限る。履歴に投稿の記録がある日・`validate` がその日のログを読み直して `not-posted` にならない日は指定できない（検証で NG） |
...
- **コミュニティ検索だけ失敗する件（2026-09-23 調査）**: PR #160 の検証 run 35808047448 で、公式 2 アカウントは取れたのに `community search` だけ失敗した（`Failed to init ClientTransaction: 'NoneType' object has no attribute 'group'`）。原因は X の IP 制限ではなく **twitter-cli 0.7.0 の初期化**: 検索（SearchTimeline）に要るヘッダー `X-Client-Transaction-Id` を作るため、cookie なしで `https://x.com` を取りに行き、そのページから `ondemand.s` の JS の場所を探すが、いまの `x.com` のページ（約 32KB）にはそれが無い（`x.com/home`（約 300KB）にはある）。見つからないと警告だけ出してヘッダーなしで続け、検索だけが弾かれる（投稿一覧はヘッダーなしでも通る）。オーナーの Mac で取れていたのは、ローカルの twitter-cli が 2026-08-26 に `x.com` → `x.com/home` と 1 行手直しされていたため（CI は手直し前の PyPI 版）。最新の 0.8.5（2026-03-17）と上流 main もまだ `x.com` を取りに行くので、**版を上げても直らない**。
- **オーナー決定 2026-09-23 A（＋同日 ①）: CI では 1 行手直しを「x.com/home をログイン情報付きで取る」形で当てる**: 上の件の直し方。`fetch-jev-x.yml` の取得ジョブで、twitter-cli を入れた直後のステップ「Patch twitter-cli (x.com -> x.com/home with the session cookie, one line)」が、venv の `twitter_cli/client.py` の `"https://x.com", headers=ct_headers,` を `"https://x.com/home", headers=dict(ct_headers, Cookie=self._cookie_string or "auth_token=%s; ct0=%s" % (self._auth_token, self._ct0)),` に置き換える。オーナーの Mac の手直し（`x.com/home` にするだけ）に、クライアント自身のログイン情報を Cookie ヘッダーに付ける分を足したもの。Cookie の式は twitter-cli が API を呼ぶときの `_build_headers` と同じで、コードに入るのは式だけ（値は実行時に「Fetch X posts」ステップの env からクライアントに入ったもの）。置き換える前に「対象がちょうど 1 か所・`x.com/home` は 0 か所」、置き換えた後に「対象 0 か所・新しい行 1 か所・`x.com/home` 1 か所」を確かめ、外れたら run を赤にして止める（版上げで黙って手直しが外れたり、別の行を書き換えたりしないため）。このステップは cookie を受け取らない（cookie を受け取るのは今までどおり「Fetch X posts」だけ）。
  - **ログイン情報を付ける理由（2026-09-23 差し戻し 2 回目）**: `x.com/home` にするだけの手直しは当たっていたが、CI では検索が 404 のままだった（PR #162 の検証 run 35818337819）。診断 run 35818722451 で、GitHub のサーバーから cookie なしで `x.com/home` を取ると **HTTP 403**（6009 bytes、`ondemand.s` なし）と確定。ローカル（家の回線）では cookie なしでも取れるので、GitHub のサーバーからだけ弾かれている。オーナー決定 ①（既に登録済みの `TWITTER_AUTH_TOKEN` / `TWITTER_CT0` をこの 1 ページの取得にも使う。新しい Secret は作らない）で、ログイン情報付きで取る形にした。1 回だけの試行で、検証 run でも検索が失敗したら、決定どおり ②（コミュニティ検索はあきらめてウェブ検索で補う）に切り替える。診断用に一時的に足していた「Diagnose X search header init (no cookie)」ステップは、cookie なしでは 403 が出続けるだけなので消した。`errors` には twitter-cli の WARNING 行（`Failed to init ClientTransaction: …` など）も ` [warning: …]` として続けて入る。
  - **版を上げるとき**: 新しい版を一時 venv に入れて `grep -n '"https://x.com"' …/twitter_cli/client.py` で該当行を見る。①同じ行がそのままあれば手直しステップはそのまま使える ②書き方が変わって置換ステップが赤になったら、上流の該当行（`ClientTransaction` の初期化で `ondemand.s` を探すために取りに行くページ）に合わせてステップの `old` / `new` を直す ③上流が `x.com/home`（か別の直し方）に直っていたら、手直しステップを消す（そのままだと「対象 0 か所」で赤になる）。
  - それでも `errors` に `community search: …` が残る日（X 側の変更など）は、他の人の使用例は X から入らない — **朝ルーチンは使用例の題材を WebSearch（開発者ブログ・GitHub・Hacker News・Reddit・`site:x.com` 検索）で補う手順が [routine-prompt-jev.md](routine-prompt-jev.md) の手順 3「第2段階」にある**（見つからない日は解説回で投稿は止まらない）。失敗理由は 2026-09-23 から twitter-cli の JSON のエラー文（`error.code: message`）が `errors` に入る（以前は警告の 1 行目しか残らず原因が読めなかった。cookie の値は伏せる）
...
> **不採用（2026-09-23 オーナー決定: チャンネルは AI 全般、Jev は特集。プロフィール・説明文は変えない）**。YT「HAL- AI情報カフェ」と IG は AI 全般のチャンネルで、Jev は「いま話題の AI モデル」として取り上げる特集にすぎない。動画・タイトル・キャプション・ハッシュタグも「このチャンネル＝Jev」と読める書き方をしない（`scripts/jev.mjs` の特集表記とテストで固定）。以下の案は記録として残すだけで、反映しない。
