...
- **コミュニティ検索だけ失敗する件（2026-09-23 調査）**: PR #160 の検証 run 35808047448 で、公式 2 アカウントは取れたのに `community search` だけ失敗した（`Failed to init ClientTransaction: 'NoneType' object has no attribute 'group'`）。直接の原因は **twitter-cli 0.7.0 の初期化**（ただし CI では、下の「試したこと」のとおり GitHub のサーバーから見た X の応答も違い、手直しでは直らなかった）: 検索（SearchTimeline）に要るヘッダー `X-Client-Transaction-Id` を作るため、cookie なしで `https://x.com` を取りに行き、そのページから `ondemand.s` の JS の場所を探すが、いまの `x.com` のページ（約 32KB）にはそれが無い（`x.com/home`（約 300KB）にはある）。見つからないと警告だけ出してヘッダーなしで続け、検索だけが弾かれる（投稿一覧はヘッダーなしでも通る）。オーナーの Mac で取れていたのは、ローカルの twitter-cli が 2026-08-26 に `x.com` → `x.com/home` と 1 行手直しされていたため（CI は手直し前の PyPI 版）。最新の 0.8.5（2026-03-17）と上流 main もまだ `x.com` を取りに行くので、**版を上げても直らない**。
- **CI で直そうとして試したこと（2026-09-23、どちらも効かなかった）**:
  1. `x.com` → `x.com/home` の 1 行手直し（オーナー決定 A、オーナーの Mac と同じ手直し）: CI では効かない。GitHub のサーバーから cookie なしで `x.com/home` を取ると **HTTP 403**（6009 bytes、`ondemand.s` なし。診断 run 35818722451）。家の回線からは cookie なしでも取れる
  2. `x.com/home` をログイン情報（`TWITTER_AUTH_TOKEN` / `TWITTER_CT0`）付きで取る手直し（オーナー決定 ①、1 回だけの試行）: 手直しは当たったが、`ClientTransaction` の初期化は同じく失敗し（`'NoneType' object has no attribute 'group'`）、検索は 404 のまま（PR #165 の検証 run 35862195121）。公式 2 アカウントは 30 件ずつ取れ、アカウントのロック・確認要求・制限系のエラーは出ていない
- **オーナー決定 2026-09-23 ②: CI での X のコミュニティ検索はあきらめる**。ワークフローには手直しを入れず、twitter-cli は PyPI 版（0.7.0、ハッシュ固定）のまま使う。**`data/jev-x-posts.json` の `errors` に `community search: …` が毎日残るのは想定どおり**（run は赤にならない。赤になるのは全部の取得が失敗したとき = ログイン切れ）。他の人の使用例は X から入らないので、**朝ルーチンが使用例の題材を WebSearch（開発者ブログ・GitHub・Hacker News・Reddit・`site:x.com` 検索）で補う手順が [routine-prompt-jev.md](routine-prompt-jev.md) の手順 3「第2段階」にある**（見つからない日は解説回で投稿は止まらない）。
  - `errors` の読み方: 2026-09-23 から、twitter-cli の JSON のエラー文（`error.code: message`）と WARNING 行（`Failed to init ClientTransaction: …` など）が ` [warning: …]` として `errors` に入る（以前は警告の 1 行目しか残らず原因が読めなかった。cookie の値は伏せる）。公式 2 アカウントの行に失敗が出たら、コミュニティ検索とは別の問題として見る
  - 版を上げるとき・上流が直ったとき: 新しい版を一時 venv に入れ、ローカルで一時 HOME（キャッシュなし）にしてコミュニティ検索が通るかを見てから、PR の検証 run で `community search: N posts` が出るかを確かめる。出れば、この節の「あきらめる」を外す
