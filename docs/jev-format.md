...
| 前の回に投稿の記録が無い（`performance-history.json` に YouTube・Instagram どちらの記録も無い） | **記録が無いだけでは「放送済み」のまま**（`next` の `unrecorded` に出る）。投稿は成功したのに記録だけ失敗した日にやり直すと二重投稿になるため。`node scripts/jev.mjs aired-check <その日>` がその日の投稿ワークフローのログに投稿成功の行（YouTube「Uploaded!」／Instagram「Published!」）が無いことを確かめて `not-posted` と出たときだけ、`next` の `redoSlot` の枠を **`redo_of: "<その日>"` 付き**で書き直す。`redo_of` で指された回は**期限なく永久に**シリーズから外れる（時間で戻ることはない）。やり直せるのは最新の回だけで、`posted` / `unknown` のときや、もう次の回が出ているときはやり直さない（レポートに書く） |
...
| `redo_of`（任意） | 投稿されなかった最新の回をやり直すときだけ、その回の日付。`aired-check` が `not-posted` と出たときに限る。履歴に投稿の記録がある日は指定できない（検証で NG） |
